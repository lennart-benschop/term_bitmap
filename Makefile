FFLAGS=-DUSE_LINUX_FB
CFLAGS=-O3 -Wall -I ./incl $(FFLAGS)

default: lib tests

lib: libtbm.a

libtbm.a: src/tbm_main.o src/tbm_plot.o src/tbm_text.o src/tbm_out_sixel.o src/tbm_out_kitty.o src/tbm_out_octant.o src/tbm_out_sextant.o src/tbm_out_braille.o src/tbm_out_fb.o src/tbm_probe.o src/tbm_palette.o src/tbm_bitmap.o
	$(AR) r $@ src/*.o
	ranlib libtbm.a

tests: test_simple test_cube test_dodec test_mandel

test_simple: tests/test_simple.o libtbm.a
	$(CC) -o $@ $^ -L . -ltbm -lm
test_mandel: tests/test_mandel.o libtbm.a
	$(CC) -o $@ $^ -L . -ltbm -lm
test_cube: tests/test_cube.o libtbm.a
	$(CC) -o $@ $^ -L . -ltbm -lm
test_dodec: tests/test_dodec.o libtbm.a
	$(CC) -o $@ $^ -L . -ltbm -lm

clean:
	rm -f test_cube test_dodec test_simple test_mandel libtbm.a src/*.o tests/*.o

